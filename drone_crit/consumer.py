...
def handle_event(id, details_str):
    ...
    try:
        ...
        if details['operation'] == 'cancel_command':
            details['operation'] = 'stop'
            details['deliver_to'] = 'flight_controller'
            proceed_to_deliver(id, details)
            details['operation'] = 'off_drives'
            details['deliver_to'] = 'drone_battery_control'   
            proceed_to_deliver(id, details)
            details['operation'] = 'critical_situation'
            details['deliver_to'] = 'drone_ccu'
            proceed_to_deliver(id, details)
            #delivery_required = True
        else:
            print(f"[warning] unknown operation in drone_crit!\n{details}")                
